...
class StdEMM:
    def __init__(self):
        self.par_lambda = DEFAULT_par_lambda
        self.l = DEFAULT_l
        self.k = DEFAULT_k
        self.N = DEFAULT_N
        self.v = DEFAULT_v
        self.n = DEFAULT_n
        self.t = DEFAULT_t
        self.N_lambda = DEFAULT_N_lambda
        self.n_lambda = DEFAULT_n_lambda
...
class BlackboxORAM:
    ...
    def PrintClientState(self):
        state = (self.k + math.log2(self.t_star)) * self.t_star + self.l_star * self.v * math.log2(self.t_star)
...
class ZAVLH:
    def __init__(self):
        self.par_gates = DEFAULT_PAR_GATES # used for the constant in computing number of gates
        self.par_lambda = DEFAULT_par_lambda
        self.l = DEFAULT_l
        self.k = DEFAULT_k
        self.N = DEFAULT_N
    ...
    def NumGates(self):
        num = self.par_gates * (self.t + 2 * self.par_lambda) * math.log2(self.t + 2 * self.par_lambda)
        return num
    
    def ClientStateOPS(self):
        state = 2 * self.k * (self.t + self.par_lambda) + (self.k + math.log2(self.par_lambda)) * self.par_lambda + (self.l + self.par_lambda) * self.v * math.log2(self.par_lambda)
        return state
    ...
    def ClientStateUP(self):
        state = 2 * self.k * (self.t_lambda + self.par_lambda) + (self.k + math.log2(self.par_lambda)) * self.par_lambda + (self.l_lambda + self.par_lambda) * self.v * math.log2(self.par_lambda)
        return state
    ...
    def ServerStorageOPS(self):
        storage = ((self.N + self.par_lambda) * self.v + self.k * self.n) + 5 * self.v * (2 * self.par_lambda - 1) * (self.l + self.par_lambda)
        return storage
    
    def ServerStorageEandT(self):
        storage = (self.t + 2 * self.par_lambda) * (2 * self.k + (self.l + self.par_lambda) * self.v)
        return storage
    ...
    def ServerStorageUP(self):
        storage = ((self.N_lambda + self.par_lambda) * self.v + self.k * self.n_lambda) + 5 * self.v * (2 * self.par_lambda - 1) * (self.l_lambda + self.par_lambda)
        return storage

    def PrintServerStorage(self):
        storage_OPS = PrintInMbit(self.ServerStorageOPS(), "ZAVLH", "Server Storage", "OPS")
        storage_EandT = PrintInMbit(self.ServerStorageEandT(), "ZAVLH", "Server Storage", "E&T")
        storage_SandS = PrintInMbit(self.ServerStorageSandS(), "ZAVLH", "Server Storage", "S&S")
        storage_UP = PrintInMbit(self.ServerStorageUP(), "ZAVLH", "Server Storage", "UP")
    ...
    def BandwidthOPS(self):
        bandwidth = self.l * self.k + self.l * ((self.N + self.par_lambda) * self.v / self.n) + 10 * self.v * (self.l + self.par_lambda) * math.log2(self.par_lambda)
        bandwidth_total = self.par_lambda * bandwidth
        return bandwidth_total

    def BandwidthEandT(self):
        bandwidth = self.l * (self.t + self.par_lambda) * (self.k + (self.N + self.par_lambda) * self.v / self.n) + 5 * self.v * (self.l + self.par_lambda) * self.par_lambda * math.log2(self.par_lambda) + self.ServerStorageEandT()
        return bandwidth

    def BandwidthSandS(self):
        bandwidth = 4 * (2 * self.k + (self.l + self.par_lambda) * self.v)
        term_S_pow = self.NumGates()
        bandwidth_total = bandwidth * term_S_pow
        return bandwidth_total

    def BandwidthUP(self):
        bandwidth = self.k * self.n_lambda + math.log2((self.N_lambda + self.par_lambda) / self.n_lambda) + 2 * self.l_lambda * (self.t_lambda + self.par_lambda) * (self.k + (self.N_lambda + self.par_lambda) * self.v / self.n_lambda) + self.ServerStorageEandT()
        return bandwidth
...
if __name__ == "__main__":
    ...
    oram = BlackboxORAM(growth_factor)
